...
trait Swal
{
    ...
    {
        $this->dispatchBrowserEvent('swal:toast', ['icon' => 'success', 'title' => $title, 'text' => $text, 'opts' => $opts]);
    }
    protected function swalToastWarning(string $title = 'Warning', string $text = '', array $opts = []): void
    {
        $this->dispatchBrowserEvent('swal:toast', ['icon' => 'warning', 'title' => $title, 'text' => $text, 'opts' => $opts]);
    }
    protected function swalToastError(string $title = 'Error', string $text = '', array $opts = []): void
    {
        $this->dispatchBrowserEvent('swal:toast', ['icon' => 'error', 'title' => $title, 'text' => $text, 'opts' => $opts]);
    }

    // Modal (larger)
    protected function swalFireSuccess(string $title = 'Success', string $text = '', array $opts = []): void
    {
        $this->dispatchBrowserEvent('swal:modal', ['icon' => 'success', 'title' => $title, 'text' => $text, 'opts' => $opts]);
    }
    protected function swalFireWarning(string $title = 'Warning', string $text = '', array $opts = []): void
    {
        $this->dispatchBrowserEvent('swal:modal', ['icon' => 'warning', 'title' => $title, 'text' => $text, 'opts' => $opts]);
    }
    protected function swalFireError(string $title = 'Error', string $text = '', array $opts = []): void
    {
        $this->dispatchBrowserEvent('swal:modal', ['icon' => 'error', 'title' => $title, 'text' => $text, 'opts' => $opts]);
    }
    ...
    protected function swalPromptPassword(
        ...
        $this->dispatchBrowserEvent('swal:password', [
            'title' => $title,
            'text' => $text,
            'thenEvent' => $thenEvent,
            'thenParams' => $thenParams,
    ...
    protected function swalVerifyCurrentUserPassword(
        ...
        $this->dispatchBrowserEvent('swal:verify-current-password', [
            'title' => $title,
            'text' => $text,
